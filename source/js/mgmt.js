...
(function(){
  ...
  function openScorecardModal(t){
    ...
    const holeCell = (sc, hi) => {
      ...
      const p = pars[hi];
      const style = v < p ? 'color:#7ee8b8;font-weight:700' : (v > p ? 'color:#ff9d9d' : '');
      return `<td class="num" style="${style}">${D.fa(v)}</td>`;
    };
    m.innerHTML = `
    <div class="glass gold-border" style="width:min(920px,96vw);max-height:92vh;overflow:auto;padding:20px 22px">
      <div class="card-head"><span class="ic">📋</span><h3>اسکورکارت «${esc(t[1])}»</h3>
        <span class="tag">${D.fa(j.dd)} ${j.monthFa} • ${esc(D.COURSE_NAME[t[3]] || '—')} • ${D.fa(t[4] || pars.length)} حفره • پار ${D.fa(parTotal)}</span></div>
      ${cards.length ? `
