...
(function(){
  ...
  function mgmtCalendar(body){
    ...
    function openEventParticipants(i){
      const lst = customEvents();
      const e = lst[i]; if (!e) return;
      const pidSet = new Set((e.participants || []).map(Number));
      let m = $('#modal-evpart');
      if (!m){
        m = document.createElement('div');
        m.id = 'modal-evpart';
        m.style.cssText = 'position:fixed;inset:0;z-index:210;display:none;align-items:center;justify-content:center;background:rgba(4,8,14,.72);backdrop-filter:blur(6px)';
        document.body.appendChild(m);
        m.addEventListener('click', ev2 => { if (ev2.target === m) m.style.display = 'none'; });
      }
      m.innerHTML = `
      <div class="glass gold-border" style="width:min(560px,94vw);padding:20px;max-height:86vh;overflow:auto">
        <div class="card-head"><span class="ic">👥</span><h3>لیست شرکت‌کنندگان — ${esc(e.name)}</h3><span class="tag">🎯 هر نفر ${D.fa(e.pts||0)} امتیاز</span></div>
        <div style="font-size:11px;color:var(--muted);margin-top:6px">تیک بزنید — فوراً ذخیره می‌شود؛ امتیاز رویداد در جدول «نمایش تلویزیونی (مجموع امسال)» حساب می‌شود، نه رقابتِ فقط‌مسابقات.</div>
        <div id="evp-list" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:7px;margin-top:12px">
          ${activePlayersList().map(p => `<label style="display:flex;align-items:center;gap:8px;background:rgba(255,255,255,.03);border:1px solid var(--line-soft);border-radius:10px;padding:8px 10px;cursor:pointer">
            <input type="checkbox" data-evpid="${p.pid}" ${pidSet.has(+p.pid) ? 'checked' : ''} style="accent-color:#d4af37;width:16px;height:16px">
            <span style="font-size:12px;font-weight:700">${esc(p.name)}</span>
          </label>`).join('')}
        </div>
        <div style="display:flex;gap:8px;margin-top:14px;justify-content:flex-end">
          <span id="evp-count" style="color:var(--gold-l);font-size:12px;align-self:center;margin-left:auto"></span>
      ...
      const sync = () => {
        const picked = [];
        m.querySelectorAll('[data-evpid]').forEach(cb => { if (cb.checked) picked.push(+cb.dataset.evpid); });
        const l2 = customEvents();
        if (l2[i]){ l2[i].participants = picked; saveEvents(l2); }
        m.querySelector('#evp-count').textContent = `✔ ${D.fa(picked.length)} نفر × ${D.fa(customEvents()[i].pts||0)} امتیاز = ${D.fa(picked.length * (customEvents()[i].pts||0))} امتیاز`;
      };
      m.querySelectorAll('[data-evpid]').forEach(cb => cb.addEventListener('change', () => { sync(); if (window.APP) APP.reloadData(); }));
