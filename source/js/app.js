...
(function(){
    ...
    setTimeout(() => {
      ...
      if (sp) sp.addEventListener('click', () => {
        ...
        igStory(sp, 'استوری-سکوی-قهرمانی.png', ({ W, H, GOLD, GL, FG, MUT, rrect, txt, c }) => {
          ...
          txt('FedEx Style  •  ' + D.fa(D.seasonYear), W / 2, 486, '500', 22, 'rgba(212,175,55,.7)');
          /* چیدمان مثل صفحه: چپ ۳، وسط ۱، راست ۲ — پله با نام و امتیاز داخل سکو */
          const visual = [top3[2], top3[0], top3[1]];
          const cw = 280, gap = 36;
          const x0 = (W - (cw * 3 + gap * 2)) / 2;
          const floor = 1540;
          const baseH = [300, 520, 390];
          const medalCol = [
            ['#f0c49a', '#c67b3a', '#8a4e1c'],
            ['#ffe38a', '#d4af37', '#8c6e1d'],
            ['#f4f7fb', '#b8c2cc', '#7d8792']
          ];
  ...
  /* آخرین بازیکنی که در جلسهٔ باز ضربه ثبت کرده — تحلیل لحظه‌ای باید همین را نشان بدهد نه سلکت بالای صفحه */
  function lastOpenShotPid(){
    try {
      const ssn = spSessions();
      const open = Object.keys(ssn).map(k => ssn[k]).find(x => x && x.status === 'open');
      if (!open) return null;
      const shots = spShots().filter(x => String(x.sid) === String(open.id)).sort((a,b) => (+a.t||0) - (+b.t||0));
      if (!shots.length) return null;
      const pid = +shots[shots.length - 1].pid;
      return (A.LB || []).some(r => +r.pid === pid) ? pid : null;
    } catch(e){ return null; }
  }
    ...
    PDFK.a4({
      ...
      sections: [{ h: 'کلید رنگ‌ها', sub: 'صاف • سمت راست • سمت چپ • ضربه خراب', html: legend }].concat(
        groups.map(g => ({
          ...
          html: '<div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px">' + Object.keys(g.clubs).sort().map(cn => {
            const cl = g.clubs[cn]; const dom = paTopRes(cl);
            return '<div style="text-align:center;background:rgba(255,255,255,.02);border:1px solid rgba(212,175,55,.18);border-radius:10px;padding:8px 4px 7px;min-width:0">'
              + paSvgDonut(cl, 72, 14)
              + '<div style="font-size:9px;font-weight:800;margin-top:3px;color:#e9eef5">' + esc(cn) + '</div>'
  ...
  function renderSmartPlayer(v, p, cards, TABS){
    ...
    if (rec) rec.addEventListener('click', () => {
      ...
      if (window.SMARTPLAY) SMARTPLAY.open({ A: A, S: S, D: D, onExit: function(){
        const lp = lastOpenShotPid();
        if (lp) playerSel = lp;
        playerTab = 'smart';
        go('player');
      } });
  ...
  function renderSmartChart(p){
    ...
    const livePid = lastOpenShotPid();
    if (livePid){
      const row = A.LB.find(r => +r.pid === +livePid);
      if (row) p = row;
    }
    const mine = shots.filter(x => +x.pid === +p.pid && String(x.sid) === String(open.id)).sort((a, b) => (+a.t||0) - (+b.t||0));
    if (!mine.length){
  ...
  function pageCal(){
    ...
    function daysInJMonth(mm){
      if (mm <= 6) return 31;
      if (mm <= 11) return 30;
      const g = D.jalaaliToDateObject(D.seasonYear, 12, 30);
      const j = D.toJalaali(g.gy, g.gm, g.gd);
      return (j[0] === D.seasonYear && j[1] === 12 && j[2] === 30) ? 30 : 29;
    }
    function jalMonthStart(mm){
      const iso = D.shamsiToISO(D.seasonYear, mm, 1);
      return D.dateFrom(iso);
    }
    ...
    if (stCal) stCal.addEventListener('click', () => {
      ...
      igStory(stCal, 'استوری-تقویم-فصل.png', ({ W, H, GOLD, GL, FG, MUT, rrect, txt, c }) => {
        ...
        events.forEach(e => {
          ...
          for (let k = 0; k < n; k++){
            const di = new Date(e.d.getTime() + k * 86400000);
    ...
    function renderMonthStats(){
      ...
      const cnt = events.filter(e => {
        ...
        if (j0.yy !== D.seasonYear || j0.mm !== mm) return false;
        const n = daysBetween(e.d, e.end);
        for (let i=0;i<n;i++){ if (j0.dd + i >= 1 && j0.dd + i <= daysInJMonth(mm)) return true; }
        return false;
      }).length;
