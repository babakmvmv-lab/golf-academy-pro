...
(function(){
  ...
  function openResultsModal(opts){
    ...
    function render(){
      ...
      partBox.innerHTML = part.map(pid =>
        `<div class="rp-item sel" data-pid="${pid}">${esc(nameOf(pid))} ✕</div>`).join('') || '<div style="color:var(--muted);font-size:12px;padding:8px">با یک لمس روی نام‌های ستون چپ اینجا اضافه می‌شوند</div>';
      ['1','2','3'].forEach(k => {
  ...
  function openScoreWizard(t){
    ...
    if (!_dA[t[0]].__meta.startedAt){ _dA[t[0]].__meta.startedAt = new Date().toISOString(); scDraftSave(_dA); }
    let w = $('#score-wizard');
    if (!w){
    ...
    function renderSavedResults(){
      ...
      $('#mr-list').innerHTML = keys.length ? keys.map(tid => {
        ...
        return `<div style="background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:10px 13px;margin-bottom:8px;display:flex;align-items:center;gap:9px;flex-wrap:wrap;font-size:11.5px">
          ...
          <span class="chip dim">👥 ${D.fa((res.participants || []).length)} شرکت‌کننده</span>
