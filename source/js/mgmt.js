...
(function(){
  ...
  function mgmtCalendar(body){
    ...
    <div class="glass gold-border" style="margin-bottom:16px">
      ...
      <div class="field-grid" style="margin-top:10px">
        ...
        <div><label>🎯 امتیاز هر شرکت‌کننده</label><input class="input" id="me-pts" type="number" min="0" value="5" style="width:100%"></div>
      </div>
      <div class="field-grid" style="margin-top:10px">
    ...
    $('#me-add').addEventListener('click', () => {
      ...
      const obj = { name: name || 'رویداد ' + D.fa(lst.length+1), type: $('#me-type').value, col: $('#me-col').value,
        pts: Math.max(0, +($('#me-pts').value || 0)), start, end, schedule };
      ...
      editingIdx = -1; setBtn();
      $('#me-name').value = '';
      $('#me-type').value = 'مسابقه'; $('#me-col').value = 'green'; $('#me-pts').value = 5;
      renderMeList();
    });

    /* 👥 مودال انتخاب شرکت‌کنندگان رویداد — بازیکن انتخاب‌شده امتیاز رویداد را در «مجموع امسال» (TV) دریافت می‌کند */
    /* 👥 مودال انتخاب شرکت‌کنندگان رویداد — تک‌لمس (سبک «نتایج مسابقات»): لیست همه در بالا، انتخاب‌شده‌ها می‌آیند پایین */
    function openEventParticipants(i){
      const lst0 = customEvents();
      let e = lst0[i]; if (!e) return;
      const all = activePlayersList(); /* همهٔ بازیکنان فعال */
      let part = (e.participants || []).map(Number);
      let m = $('#modal-evpart');
      if (!m){
        m = document.createElement('div');
        m.id = 'modal-evpart';
        m.style.cssText = 'position:fixed;inset:0;z-index:9501;display:none;align-items:center;justify-content:center;background:rgba(4,8,14,.78);backdrop-filter:blur(6px)';
        document.body.appendChild(m);
        m.addEventListener('click', ev2 => { if (ev2.target === m) { m.style.display = 'none'; renderMeList(); } });
      }
      function nameOf(pid){ const x = all.find(p => +p.pid === +pid); return x ? x.name : '—'; }
      function saveLive(){
        const l2 = customEvents();
        if (l2[i]){ l2[i].participants = part.slice(); saveEvents(l2); }
        if (window.APP) APP.reloadData(); /* امتیازها در TV هم‌درنگ */
      }
      function render(){
        const cur = customEvents()[i] || e;
        const pts = +cur.pts || 0;
        const q = (m.querySelector('#evp-search').value || '').trim();
        const partSet = new Set(part);
        m.querySelector('#evp-all').innerHTML = all.filter(p => !partSet.has(+p.pid) && (!q || p.name.includes(q))).map(p =>
          `<div class="rp-item" data-pid="${p.pid}">${esc(p.name)}</div>`).join('') || '<div style="color:var(--muted);font-size:12px;padding:8px">— همه شدند —</div>';
        m.querySelector('#evp-part').innerHTML = part.map(pid =>
          `<div class="rp-item sel" data-pid="${pid}">${esc(nameOf(pid))} ✕</div>`).join('') || '<div style="color:var(--muted);font-size:12px;padding:8px">با یک لمس روی نام‌های بالا اضافه می‌شوند</div>';
        m.querySelector('#evp-head').textContent = '✅ شرکت‌کنندگان (' + D.fa(part.length) + ' نفر)';
        m.querySelector('#evp-count').textContent = `🎯 ${D.fa(part.length)} نفر × ${D.fa(pts)} امتیاز = ${D.fa(part.length * pts)}`;
      }
      m.innerHTML = `
      <div class="glass gold-border" style="width:min(680px,94vw);padding:20px;max-height:88vh;overflow:auto">
        <div class="card-head"><span class="ic">👥</span><h3>لیست شرکت‌کنندگان — ${esc(e.name)}</h3><span class="tag">یک ✋ لمس = جابه‌جایی</span></div>
        <div style="display:flex;align-items:center;gap:8px;margin-top:8px;flex-wrap:wrap">
          <input class="input" id="evp-search" placeholder="🔎 جست‌وجوی بازیکن…" style="flex:1;min-width:150px" autocomplete="off">
          <button type="button" class="btn sm ghost" id="evp-allin" style="font-size:10px;padding:3px 9px">✓ همه</button>
          <button type="button" class="btn sm ghost" id="evp-none" style="font-size:10px;padding:3px 9px">✕ هیچ</button>
        </div>
        <div class="lbl" style="margin-top:10px">👥 همهٔ بازیکنان</div>
        <div id="evp-all" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:6px;margin-top:6px"></div>
        <div class="lbl" id="evp-head" style="margin-top:14px;color:var(--gold-l)"></div>
        <div id="evp-part" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:6px;margin-top:6px"></div>
        <div style="display:flex;gap:8px;margin-top:14px;align-items:center;justify-content:flex-end">
          <span id="evp-count" style="color:var(--gold-l);font-size:12px;margin-left:auto"></span>
          <button class="btn sm ghost" id="evp-x">بستن</button>
        </div>
        <div style="font-size:10px;color:var(--muted);margin-top:8px">ذخیرهٔ فوری ✔ — امتیاز رویداد به جدول «نمایش تلویزیونی (مجموع امسال)» می‌رود، نه رقابتِ فقط‌مسابقات.</div>
      </div>`;
      m.style.display = 'flex';
      const allBox = m.querySelector('#evp-all'), partBox = m.querySelector('#evp-part');
      allBox.onclick = ev3 => {
        const it = ev3.target.closest('.rp-item'); if (!it) return;
        part.push(+it.dataset.pid); saveLive(); render();
      };
      partBox.onclick = ev3 => {
        const it = ev3.target.closest('.rp-item'); if (!it) return;
        part = part.filter(p => p !== +it.dataset.pid); saveLive(); render();
      };
      m.querySelector('#evp-search').oninput = () => render();
      m.querySelector('#evp-allin').onclick = () => { part = all.map(p => +p.pid); saveLive(); render(); };
      m.querySelector('#evp-none').onclick = () => { part = []; saveLive(); render(); };
      m.querySelector('#evp-x').onclick = () => { m.style.display = 'none'; renderMeList(); };
      render();
    }
    ...
    $('#me-list').innerHTML = lst.length ? `<div class="holi-list">
      ${lst.map((e,i) => {
        ...
        return `<div class="h-item" style="align-items:flex-start">
          ...
          <span style="flex:1"><b>${esc(e.name)}</b> <span class="chip gold" title="امتیازی که هر شرکت‌کننده می‌گیرد">🎯 ${D.fa(e.pts||0)} امتیاز</span>
            <div style="font-size:10.5px;color:var(--muted)">${esc(e.type||'')}${n>1?' • '+D.fa(n)+' روز':''}${sch?' • '+sch:''}${(e.participants||[]).length?' • 👥 '+D.fa(e.participants.length)+' شرکت‌کننده':''}</div>
          </span>
          <button class="btn sm ghost" data-partme="${i}" title="انتخاب بازیکنان شرکت‌کننده — هرکدام ${D.fa(e.pts||0)} امتیاز می‌گیرند">👥</button>
          <button class="btn sm ghost" data-edme="${i}" title="ویرایش (نام، نوع، رنگ، امتیاز، تاریخ)">✏️</button>
    ...
    $$('#me-list [data-partme]').forEach(b => b.addEventListener('click', () => openEventParticipants(+b.dataset.partme)));
    $$('#me-list [data-edme]').forEach(b => b.addEventListener('click', () => {
      ...
      $('#me-pts').value = (e.pts !== undefined && e.pts !== null) ? e.pts : 5;
  ...
  function mgPdfLibs(){
    ...
    if (!_pdfLibsP2){
      /* چند CDN پشت‌سرهم — اگر جی‌اس‌دلیور روی شبکهٔ کاربر بسته بود، unpkg/cdnjs در می‌آیند */
      const once = src => new Promise((res, rej) => {
        if ([...document.scripts].some(s => s.src === src)) return res();
        const sc = document.createElement('script'); sc.src = src; sc.async = true;
        const tm = setTimeout(() => { sc.remove(); rej(new Error('cdn-timeout')); }, 15000);
        sc.onload = () => { clearTimeout(tm); res(); };
        sc.onerror = () => { clearTimeout(tm); sc.remove(); rej(new Error('cdn')); };
        document.head.appendChild(sc);
      });
      const tryAll = (urls, test) => urls.reduce((p, u) => p.catch(() => test() ? Promise.resolve() : once(u).then(() => {
        if (!test()) return Promise.reject(new Error('bad-lib'));
      ...
      _pdfLibsP2 = tryAll(H2C, () => !!window.html2canvas)
        .then(() => tryAll(JSPDF, () => !!(window.jspdf && window.jspdf.jsPDF)));
  ...
  function storyReportPng(t, btn){
    ...
    const fa = v => D.fa(v);
    const medals = ['🥇', '🥈', '🥉'];
    const list2 = extraCards().filter(c => c.tour === t[0]).map(c => {
    ...
    const build = () => {
      ...
      const drawRest = () => {
        const doneLogo = () => {
          ...
          if (top3.length){
            const cardW = 316, baseY = 760;
            top3.forEach((r, i) => {
              const is1 = r.rank === 1;
              const chH = is1 ? 400 : 340, cw = cardW;
            ...
            txt('نتایج این مسابقه هنوز ثبت نهایی نشده', W / 2, 860, 600, 30, MUT);
            txt('پس از پایان، جدول رتبه‌بندی همین‌جا می‌آید ⛳', W / 2, 910, 500, 24, 'rgba(139,150,164,.6)');
          }
          /* سایر رتبه‌ها */
          ...
          if (rest.length){
            ...
            yy += 20;
            cols.forEach(cl => txt(cl[1], cl[0], yy, 800, 23, MUT, 'center'));
  ...
  function tourReport(t, onChange){
    ...
    function render(){
      ...
      const sb = $('#tr-story');
      if (sb) sb.addEventListener('click', () => storyReportPng(t, sb));
    }
    render();
  }
  ...
  function openScoreWizard(t){
    ...
    function headerHtml(){
      ...
      return `<div style="display:flex;align-items:center;flex-wrap:wrap;gap:8px;padding:10px 12px;border-bottom:1px solid var(--line-soft);background:rgba(255,255,255,.02)">
        ...
        <div style="display:flex;align-items:center;gap:6px">
          ...
          <button class="btn sm ghost sw-mini" id="sw-peek" title="جدول اسکورکارت (نمایش)">👁 جدول</button>
          <button class="btn sm sw-mini" id="sw-report" style="background:linear-gradient(135deg,var(--gold),#b08a28);color:#1a1407;font-weight:800" title="ثبت نهایی و گزارش پایانی مسابقه">🏁 گزارش مسابقه</button>
        </div>
      </div>
      <div style="display:flex;gap:6px;padding:8px 12px;font-size:10px;color:var(--muted);border-bottom:1px solid var(--line-soft);flex-wrap:wrap">
    ...
    function stepPlayers(){
      ...
      return `<div style="padding:12px 10px;overflow:auto;flex:1">
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f3f4f6;padding:24px 12px"><tr><td align="center">
  ...
  function mgmtMessages(body){
    ...
    $('#ec-save').addEventListener('click', () => {
      const c = { key: $('#ec-key').value.trim(), svc: $('#ec-svc').value.trim(), tpl: $('#ec-tpl').value.trim() };
      if (!c.key || !c.svc || !c.tpl){ APP.toast('هر سه مقدار EmailJS لازم است', 'red'); return; }
      saveEmailCfg(c); APP.toast('اتصال ایمیل ذخیره شد ✓', 'green');
    });
    $('#pm-send').addEventListener('click', async () => {
      ...
      const btn = $('#pm-send'); const txt0 = btn.textContent;
      try {
        /* ── ایمیل یک‌طرفه ── */
        if (chan !== 'panel'){
          const c = emailCfg();
  ...
  function mgmtResults(body){
    ...
    toursBox.innerHTML = openTours.length ? openTours.map(t => {
      ...
      return `<div class="mr-tour-card ${has?'done':''}" data-tour="${t[0]}">
        ...
        <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap">
          ...
          <button class="btn sm" data-mrstory2="${t[0]}" title="عکس استوری اینستاگرام (۱۰۸۰×۱۹۲۰)" style="background:linear-gradient(135deg,#d62976,#fa7e1e);color:#fff;font-weight:800;font-size:10px;padding:4px 9px">📱 استوری</button>
    ...
    $$('#mr-tours [data-mrstory2]').forEach(b => b.addEventListener('click', () => {
      const t = S.tournaments.find(x => x[0] === +b.dataset.mrstory2);
      if (t) storyReportPng(t, b);
    }));
    ...
    function renderSavedResults(){
      ...
      $$('#mr-list [data-mrstory]').forEach(b => b.addEventListener('click', () => {
        const t = S.tournaments.find(x => x[0] === +b.dataset.mrstory);
        if (t) storyReportPng(t, b);
      }));
    }
  }
