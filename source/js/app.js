...
(function(){
  ...
  const v = $('#view');
    ...
    /* حفره‌به‌حفره: خودکار روی آخرین مسابقهٔ دارای کارتِ همین بازیکن (سلکتور مسابقه حذف شد) */
    try {
      const myTours = (A.CARDS[p.pid] || []).map(c => c.tour);
      const past = S.tournaments.filter(t => D.dateFrom(t[5]) < D.TODAY && myTours.includes(t[0]));
      if (past.length) matchSel = past[past.length - 1][0];
    } catch(e){}
    setTimeout(() => {
