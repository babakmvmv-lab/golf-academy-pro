...
(function(){
  ...
  function finalizeTournament(t){
    ...
    const r = D.loadResults();
    r[t[0]] = Object.assign(r[t[0]] || {}, { participants: [], top: {}, auto: true, ts: new Date().toISOString().slice(0, 10) });
    D.saveResults(r);
    ...
    function render(){
      ...
      rp.innerHTML = `<div style="max-width:760px;width:100%;margin:0 auto;padding:18px 14px 24px" id="tr-capture">
        ...
        <table class="tbl" style="width:100%;margin-top:14px">
          <thead><tr><th style="width:54px">رتبه</th><th>بازیکن</th><th>میدان</th><th>مجموع ضربه</th><th>مجموع پار</th><th>نسبت به پار</th><th data-no-pdf style="width:158px">اقدام (بعد ثبت)</th></tr></thead>
          <tbody>
    ...
    function renderSavedResults(){
      ...
      $('#mr-list').innerHTML = keys.length ? keys.map(tid => {
        ...
        return `<div style="background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:12px;margin-bottom:10px">
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><b>${esc(t[1])}</b><span class="chip green">ثبت شده</span><span style="font-size:10.5px;color:var(--muted)">${D.fa(res.participants.length)} شرکت‌کننده</span>
            <span style="flex:1"></span>
            <button class="btn sm ghost" data-mrrep="${tid}" title="گزارش مسابقه + جریمه/حذف بازیکن">📊 گزارش</button>
          </div>
