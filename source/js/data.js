/* ═══════════════ پات کلاب • Putt Club Golf Academy — Data engine (seeded, live) ═══════════════ */
(function(){
  ...
  function isoToShamsi(iso){
    ...
    const [jy, jm, jd] = toJalaali(tp.y, tp.m, tp.d);
  ...
  function tehranParts(input){
    ...
    try {
      ...
      }).formatToParts(src).forEach(p => { map[p.type] = p.value; });
    } catch (e) {
      map.year = src.getFullYear();
      map.month = String(src.getMonth()+1).padStart(2,'0');
      map.day = String(src.getDate()).padStart(2,'0');
      map.hour = String(src.getHours()).padStart(2,'0');
      map.minute = String(src.getMinutes()).padStart(2,'0');
      map.second = String(src.getSeconds()).padStart(2,'0');
    }
    let h = parseInt(map.hour, 10); if (h === 24) h = 0;
    return { y:+map.year, m:+map.month, d:+map.day, h:h, min:+map.minute, s:+(map.second||0) };
  }
  function tehranISODate(input){
    const t = tehranParts(input);
    return t.y + '-' + String(t.m).padStart(2,'0') + '-' + String(t.d).padStart(2,'0');
  }
  function tehranWeekday(input){
  ...
  const dayFmt = d => tehranISODate(d);
  ...
  Object.defineProperty(window.Data, 'TODAY', {
    get: todayStart,
    enumerable: true
  });
  seedSeason();
